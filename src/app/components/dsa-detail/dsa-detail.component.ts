...
export class DsaDetailComponent implements OnInit {
  ...
  public customersDisplayColumns = ["id", "Name", "birthdate", "gender", "panCardNo", "temporaryCode", "action"];
  ...
  public walletColumns = ['coin', 'rewardType', 'isWithdrawal', 'createdDate'];
  public partnerCommission = ['id', 'type', 'createdDate', 'commission'];
  ...
  public isSticky (action: string): boolean {
    ...
    try {
      ...
      if (res && res.status == 200) {
        if (res.recordList && res.recordList.length > 0)
          ...
          this.dsas.isDisabled = this.dsas.isDisabled ? true : false;
        if (this.dsas.basicDetail.workAddressCityId && this.dsas.basicDetail.pincode) {
  ...
  public async getCityByPincode(n) {
    try {
      this.spinnerService.show();
