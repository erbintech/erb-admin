...
export class EmployeeComponent implements OnInit {
  ...
  public employee: Dsa[] = new Array<Dsa>();
  ...
  constructor(
    ...
    if (sessionStorage.getItem("PagePermission") && JSON.parse(sessionStorage.getItem("PagePermission")).length > 0) {
      ...
      let ind = this.userPagePermission.findIndex(c => c.name == 'employee');
      if (ind >= 0) {
  ...
  public async getEmployee(pageNumber?: number) {
    try {
      this.spinnerService.show();
      ...
      let res = await this.dsaService.getDsa(this.startIndex, this.fetchRecord, 'EMPLOYEE', this.token, searchString, status, badgeId, isDelete, this.dateFrom, this.dateTo);
      if (res && res.status == 200) {
        this.employee = res.recordList;
        if (this.employee && this.employee.length > 0) {
          for (let index = 0; index < this.employee.length; index++) {
            this.employee[index].code = this.employee[index].permanentCode ? this.employee[index].permanentCode : this.employee[index].temporaryCode
            this.employee[index].isDisabled = this.employee[index].isDisabled ? true : false;
          }
        }
  ...
  public verifiedPartner(element, commissionModal) {
    ...
    Swal.fire({
      title: 'Are you sure?',
      text: "You wan't " + Verified + " this Employee",
      type: 'warning',
        ...
        try {
          ...
          if (!isDisabled) {
            ...
            if (res && res.status == 200) {
              ...
              Swal.fire(
                Verified,
                'Successfully ' + Verified + " Employee",
                'success'
              )
              ...
              if (this.partner.parentPartnerId) {
                ...
                if (ParentPartnerCommissionResponse && ParentPartnerCommissionResponse.status == 200) {
                  if (ParentPartnerCommissionResponse.recordList.length > 0) {
                    ...
                    Swal.fire({
                      type: 'warning',
                      text: "Please First Insert commission for " + this.bank.find(c => c.id == this.commissionForm.get('bankId').value).name + " for " + this.services.find(c => c.id == this.commissionForm.get('serviceId').value).name + " of " + this.partner.parentPartnerTeamFullName + "",
                    }
                    )
                    const modalContent = document.querySelector('.swal2-container');
                    sessionStorage.getItem('dark-mode') == 'dark-only' ? modalContent.classList.add('dark-only-modal') : modalContent.classList.remove('dark-only-modal')
  ...
  public warningAlert(dsa: Dsa) {
    Swal.fire({
      title: 'Are you sure?',
      text: "You wan't Delete this Employee",
      type: 'warning',
      showCancelButton: true,
  ...
  public async editEmployee(element: Dsa) {
    this.router.navigate(['/employee/edit', element.id]);
  }

  public async viewDetail(element) {
    this.router.navigate(["employee/view", element.id])
  }
  ...
  public async assignBadge() {
    if (this.badgeId) {
      try {
        ...
        if (res && res.status == 200) {
          this.employee[this.employee.findIndex(c => c.id == this.partner.id)].badgeName = this.badges.find(c => c.id == this.badgeId.value).name
          this.spinnerService.hide();
          this.modalService.dismissAll();
        }
      } catch (error) {
  ...
  public async downloadCsv() {
    try {
      ...
      let badgeId = this.selectedBadge ? this.selectedBadge : null
      let isDelete = this.isDelete ? (this.isDelete == 'Yes' ? true : false) : null
      let res = await this.dsaService.getDsa(null, null, 'EMPLOYEE', this.token, searchString, status, badgeId, isDelete, this.dateFrom, this.dateTo);
      let connector = [];
      if (res && res.status == 200) {
  ...
  private downloadFile(data) {
    ...
    let fileName = 'EMPLOYEE_export_' + new Date().getDate() + "-" + (new Date().getMonth() + 1) + "-" + new Date().getFullYear() + " " + new Date().getTime();
