...
export class InstantLoanComponent implements OnInit {
  ...
  public displayColumns = ["id", "Customer", "ContactNo", "Email", "LoanAmount", "Employment", "Income", "Status", "DSACode", "rmFullName", "isDelete",'createdDate', "Action"];
  public instantLoans: AdminPersonalLoanResponse[] = new Array<AdminPersonalLoanResponse>();
  public instantLoan = new AdminPersonalLoanResponse();
  public paginate: any;
  ...
  constructor(
    ...
    if (sessionStorage.getItem("PagePermission") && JSON.parse(sessionStorage.getItem("PagePermission")).length > 0) {
      ...
      let ind = this.userPagePermission.findIndex(c => c.name == 'instant-loan');
      if (ind >= 0) {
  ...
  public async setPage(pageNumber: number) {
    if (pageNumber > 0) {
      ...
      await this.getInstantLoans();
    }
  }

  public async getInstantLoans() {
    try {
      this.spinnerService.show();
      let searchString = this.searchString ? this.searchString : null;
      let res = await this.personalLoanService.getPersonalLoans(4, null, this.startIndex, this.fetchRecord, this.token, this.dateFrom, this.dateTo, this.selectedStatus, searchString);
      if (res && res.status == 200) {
        this.instantLoans = res.recordList;
        this.count = res.totalRecords;
        this.paginate = this.paginationService.getPager(res.totalRecords, +this.activePage, this.take);
      }
      if(this.instantLoans.length > 0){
        this.isFlag = true
      } else {
        this.isFlag = false
      }
  ...
  public warningAlert(personalLoan: AdminPersonalLoanResponse) {
    Swal.fire({
      title: 'Are you sure?',
      text: "You wan't Delete this Instant Loan",
      type: 'warning',
      ...
      if (result.isConfirmed) {
        try {
          ...
          if (res && res.status == 200) {
            this.getInstantLoans();
            Swal.fire(
              'DELETE',
              'Successfully Deleted Instant Loan',
              'success'
            )
            ...
            // this.toastrService.success("Instant Loan Deleted Successfully")
          }
          this.spinnerService.hide();
        } catch (error) {
          this.spinnerService.hide();
  ...
  public viewDetail(obj: AdminPersonalLoanResponse) {
    this.router.navigate(["instant-loan/view/", obj.basicDetail.customerLoanId]);
  }
  ...
  public navigateEditLoanPage(customerloan: AdminPersonalLoanResponse) {
    ...
    let route = 'instant-loan' + '/edit/' + customerloan.basicDetail.customerId + "/" + customerloan.basicDetail.customerLoanId;
  ...
  public async downloadCsv() {
    try {
      this.spinnerService.show();
      let searchString = this.searchString ? this.searchString : null;
      let res = await this.personalLoanService.getPersonalLoans(4, null, null, null, this.token, this.dateFrom, this.dateTo, this.selectedStatus, searchString);
      let personalLoans = [];
      if (res && res.status == 200) {
  ...
  private downloadFile(data) {
    ...
    let fileName = 'InstantLoan_export_' + new Date().getDate() + "-" + (new Date().getMonth() + 1) + "-" + new Date().getFullYear() + " " + new Date().getTime();
