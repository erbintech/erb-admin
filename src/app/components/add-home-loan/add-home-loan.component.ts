...
@Component({
  selector: 'app-add-home-loan',
  templateUrl: './add-home-loan.component.html',
  styleUrls: ['./add-home-loan.component.scss']
})
export class AddHomeLoanComponent implements OnInit {
  ...
  public async getserviceEmploymentTypes() {
    try {
      this.spinnerService.show();
      let res = await this.employmentTypeService.getServiceEmploymentType(this.token, 7);
      if (res && res.status == 200) {
    ...
    try {
      ...
      if (res && res.status == 200) {
        if (permanenet == 'permanent') {
          ...
          if (this.coApplicantWorkDetails[index].coApplicantWorkDetailCities && this.coApplicantWorkDetails[index].coApplicantWorkDetailCities.length > 0) {
            if (isChangeCityId)
      ...
      if (res && res.status == 200) {
        ...
        if (res.recordList && res.recordList.length > 0) {
          ...
          if (res.recordList[0].permanentAddressDetail && res.recordList[0].correspondenceAddressDetail) {
            if ((res.recordList[0].permanentAddressDetail.addressLine1 && (res.recordList[0].permanentAddressDetail.addressLine1 == res.recordList[0].correspondenceAddressDetail.addressLine1)) &&
              (res.recordList[0].permanentAddressDetail.addressLine2 && (res.recordList[0].permanentAddressDetail.addressLine2 == res.recordList[0].correspondenceAddressDetail.addressLine2)) &&
  ...
  public async addPropertyDetail() {
    ...
    if (valid) {
        ...
        let propertyDetail
        {
          ...
          if (this.isSameAddress.value == "Yes") {
            this.permanentAddressForm.setValue({
              ...
              "addressLine2": this.correspondenceAddressForm.get('addressLine2').value ? this.correspondenceAddressForm.get('addressLine2').value : null,
      ...
      let form = this.isTransfer.value == 'Bank Transfer' ? this.transferPropertyDetailForm : this.propertyDetailForm
      Object.keys(form.controls).forEach(key => {
        form.controls[key].markAsTouched();
      });
    ...
    try {
      ...
      if (this.documents && this.documents.length > 0) {
        ...
        if (!documentError) {
          ...
          if (this.isSelect) {
            let res = await this.homeLoanService.insertHomeLoanDocuments(this.documents, this.customerLoanId, this.token);
            if (res && res.status == 200) {
              Swal.fire(
    ...
    if (this.homeLoan.loanDocuments && this.homeLoan.loanDocuments.length > 0) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            this.spinnerService.show();
            await this.personalLoanService.changeEmploymentType(this.customerLoanId, 7, this.token);
            this.spinnerService.hide();
          } catch (error) {
            this.spinnerService.hide();
            this.alertMessage = error;
